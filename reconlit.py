...
def banner():
    print("""%s%s                 
                ____                           ___ __ 
               / __ \___  _________  ____     / (_) /_         
              / /_/ / _ \/ ___/ __ \/ __ \   / / / __/
             / _, _/  __/ /__/ /_/ / / / /  / / / /_  
            /_/ |_|\___/\___/\____/_/ /_/  /_/_/\__/%s%s  V1.0%s

                # Coded By Yash Goti - @_YashGoti_%s
    """ % (Bold, R, W, Bold, Y, W))
...
def check_subdomain(subdomain):    
    try:        
        ...
        response = requests.get('https://'+subdomain, timeout=2.5)
        if range(str(response.status_code) >= '100' and str(response.status_code) <= '199'):
            print(Bold + W + "[~] https://" + subdomain + " | " + str(response.status_code) + W)
            print(Bold + B + " |-- Redirection : " + W + Bold+ Y + response.url + W)
            print(Bold + B + " |-- Server : " + W + Bold + Y + response.headers['Server'] + W)
            print(Bold + B + " |-- IP : " + W + Bold + Y + ip + W)
            check_openports(subdomain)            
        elif range(str(response.status_code) >= '200' and str(response.status_code) <= '299'):
            print(Bold + G + "[~] https://" + subdomain + " | " + str(response.status_code) + W)
            print(Bold + B + " |-- Redirection : " + W  + Bold+ Y + response.url + W)
            print(Bold + B + " |-- Server : " + W + Bold + Y + response.headers['Server'] + W)
            print(Bold + B + " |-- IP : " + W + Bold + Y + ip + W)
            check_openports(subdomain)            
        elif range(str(response.status_code) >= '300' and str(response.status_code) <= '399'):
            print(Bold + Y + "[~] https://" + subdomain + " | " + str(response.status_code) + W)
            print(Bold + B + " |-- Redirection : " + W + Bold + Y + response.url + W)
            print(Bold + B + " |-- Server : " + W + Bold + Y + response.headers['Server'] + W)
            print(Bold + B + " |-- IP : " + W + Bold + Y + ip + W)
            check_openports(subdomain)            
        elif range(str(response.status_code) >= '400' and str(response.status_code) <= '499'):
            print(Bold + R + "[~] https://" + subdomain + " | " + str(response.status_code) + W)
            print(Bold + B + " |-- Redirection : " + W + Bold + Y + response.url + W)
            print(Bold + B + " |-- Server : " + W + Bold + Y + response.headers['Server'] + W)
            print(Bold + B + " |-- IP : " + W + Bold + Y + ip + W)
            check_openports(subdomain)            
        elif range(str(response.status_code) >= '500' and str(response.status_code) <= '599'):
            print(Bold + B + "[~] https://" + subdomain + " | " + str(response.status_code) + W)
            print(Bold + B + " |-- Redirection : " + W + Bold + Y + response.url + W)
            print(Bold + B + " |-- Server : " + W + Bold + Y + response.headers['Server'] + W)
            print(Bold + B + " |-- IP : " + W + Bold + Y + ip + W)
            check_openports(subdomain)            
        ...
        try:            
            ip = socket.gethostbyname(subdomain)
            response = requests.get('http://'+subdomain, timeout=2.5)
            if range(str(response.status_code) >= '100' and str(response.status_code) <= '199'):
                print(Bold + W + "[~] https://" + subdomain + " | " + str(response.status_code) + W)
                print(Bold + B + " |-- Redirection : " + W + Bold + Y + response.url + W)
                print(Bold + B + " |-- Server : " + W + Bold + Y + response.headers['Server'] + W)
            ...
            print(R + "[" + Bold + Y + "i" + W + R +"] " + subdomain + " | Not Responding" +  W)            

def check_openports(subdomain):
    import sys
    import time
    import socket

    for dot in range(0, 10):
        print(Bold + B + " |-- Scanning Ports" + "." * dot + W, end="\r")
        time.sleep(2.5)
    ...
    try:    
        ...
        for port in [21, 22, 23, 25, 53, 80, 110, 111, 135, 139, 143, 443, 445, 993, 1723, 3306, 3389, 5900, 8080, 8443]:            
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(1)
            o = s.connect_ex((ip, port))        
            if o == 0:
                so = socket.getservbyport(port, protocol)           
                op = str(port) + "/" + protocol + " " + so
                output.append(str(op))                
            else:
                pass
    except:
        pass
    ...
    for o in range(len(output)):
        print(Bold + B + " |-- Open/filtered Port : " + W + Bold + Y + output[o] + W)
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if not silent:
        ...
        target_len = len(parsed_domain.netloc) + 20
        print(Bold + R + "=" * target_len + W)
        print(Bold + " SCANNING TARGET : %s " % parsed_domain.netloc + W)
        print(Bold + R + "=" * target_len + W)
        for dot in range(0, 4):
            print(Bold + B + "Finding Subdomains" + "." * dot + W, end="\r")
            time.sleep(2.5)
    ...
    finalsubdomains = subdomains.copy()
    finalsubdomains.clear()
    for subdomain in subdomains:
        if subdomain.find("<BR>"):
            subdomains = subdomain.split("<BR>")            
            for subdomain in subdomains:
                finalsubdomains.add(subdomain)
        else:
            finalsubdomains.add(subdomain)

    if finalsubdomains:
        finalsubdomains = sorted(finalsubdomains, key=subdomain_sorting_key)

        if savefile:
            write_file(savefile, finalsubdomains)

        if not silent:
            # print(Y + "[-] Total Unique Subdomains Found: %s" % len(finalsubdomains) + W)            
            print(Bold + G + "=" * 30 + W)
            print(Bold + " TOTAL SUBDOMAIN FOUND : %s " % len(finalsubdomains) + W)
            print(Bold + G + "=" * 30 + W)
        ...
        if ports:
            ...
            ports = ports.split(',')
            pscan = portscan(finalsubdomains, ports)
            pscan.run()

        elif not silent:
            for subdomain in finalsubdomains:                
